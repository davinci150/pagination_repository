...
class CachedTaskModel {
  final TaskModel model;
  final DateTime lastUpdated;

  CachedTaskModel({required this.model, required this.lastUpdated});

  bool isStale(Duration ttl) => DateTime.now().difference(lastUpdated) > ttl;
}

class TasksCache {
  final Map<int, CachedTaskModel> _entities = <int, CachedTaskModel>{};
  ...
  Stream<List<CachedTaskModel>> getTasksStream(
    Filter groupKey, {
    required int offset,
    required int limit,
  }) async* {
  ...
  Future<void> addAll({
    required Filter groupKey,
    required int from,
    required List<CachedTaskModel> tasks,
  }) async {
    final idx =
        Map<int, int>.from(_indexByGroup[groupKey] ?? const <int, int>{});
    for (var i = 0; i < tasks.length; i++) {
      final t = tasks[i];
      idx[from + i] = t.model.id;
      _entities[t.model.id] = t;
    }
  ...
  Future<void> update(CachedTaskModel task) async {
    _entities[task.model.id] = task;
    _updates.add(null);
  }

  Future<CachedTaskModel?> getById(int id) async {
    return _entities[id];
  }
